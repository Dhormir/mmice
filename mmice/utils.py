...
def get_shared_parsers():
    ...
    mask_parser.add_argument("-mask_type", default="random", choices=["grad", "random"])
...
def get_stage_one_parsers():
    ...
    train_parser.add_argument("-lora_dropout", default=0.1, type=float)
    train_parser.add_argument(
        "-bias", default="none", type=str, choices=["none", "all", "lora_only"]
    )
    train_parser.add_argument(
        "-task_type", default=TaskType.SEQ_2_SEQ_LM, type=TaskType
    )
...
def get_stage_two_parsers():
    ...
    generation_parser.add_argument(
        "-generate_type", default="sample", choices=["beam", "sample"]
    )
    ...
    search_parser.add_argument(
        "-max_mask_frac", default=0.55, type=float, help="Maximum mask fraction"
    )
    search_parser.add_argument(
        "-max_edit_rounds", default=3, type=int, help="Maximum number of edit rounds"
    )
    search_parser.add_argument(
        "-max_search_levels",
        default=4,
    ...
    misc_parser.add_argument(
        "-grad_pred",
        default="original",
        choices=["original", "contrast"],
        help="Whether to take gradient \
...
def get_args(stage):
    ...
    if stage not in ["stage1", "stage2"]:
        raise ValueError(
            "stage must be one of ['stage1', 'stage2'] " + f"but got value {stage}"
        )
...
def get_dataset_reader(task_name, split="train", data_dir="data"):
    ...
    if task_name not in task_options:
        ...
        return (
            load_dataset("SetFit/20_newsgroups", split=split)
            .filter(lambda example: example["label"] in set(range(16)))
            .map(clean_text)
        )
...
def format_classif_input(inp, label, lang):
    return (
        f"{LANGUAGES_FORMAT[lang]['label']}: "
        + str(label)
        + f". {LANGUAGES_FORMAT[lang]['input']}: "
        + str(inp)
    )
...
def format_multiple_choice_input(context, question, options, answer_idx):
    formatted_str = (
        f"question: {question} answer: choice {answer_idx}:"
...
def load_predictor(task, predictor_folder="trained_predictors/"):
    ...
    task_name = (
        "text-classification" if task == task_options[2] else "sentiment-analysis"
    )
    predictor = pipeline(
        task_name,
...
def load_base_editor(
    ...
    if "umt5-" in model_name:
        ...
        if lora and editor_path:
            ...
            base_model = UMT5ForConditionalGeneration.from_pretrained(
            ...
            model = UMT5ForConditionalGeneration.from_pretrained(
                editor_model_path, torch_dtype=torch.bfloat16, config=model_config
            )
            # umt5 config fix
            ...
            tokenizer = T5TokenizerFast.from_pretrained(
                model_name, model_max_length=max_length, truncation=True, padding=True
            )
        ...
        # We use legacy false to prevent warning
        # something was misplaced in version 4.36.2 because its throwing the warning anyway
        tokenizer = MyT5Tokenizer.from_pretrained(
            model_name,
            model_max_length=max_length,
            truncation=True,
            from_slow=True,
        ...
        model = BertForMaskedLM.from_pretrained(editor_model_path, config=model_config)
...
def html_highlight_diffs(orig, edited, tokenizer_wrapper):
    ...
    edited_mark_indices, num_add, num_del = get_marked_indices(
        orig_text_tok, edited_text_tok, "+"
    )
    orig_mark_indices, num_add_2, num_del_2 = get_marked_indices(
        orig_text_tok, edited_text_tok, "-"
    )
    ...
    for idx in reversed(orig_mark_indices):
        ...
        marked_original = (
            marked_original[:start]
            + "<b>"
            + marked_original[start:end]
            + "</b>"
            + marked_original[end:]
        )
