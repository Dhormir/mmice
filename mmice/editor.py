...
class Editor:
        ...
        for s_idx, s in enumerate(editable_segs):
            ...
            if len(predic_tokenized["input_ids"][0]) >= max_predic_tokens:
                trunc_es[s_idx] = self.predictor.tokenizer.decode(
                    predic_tokenized["input_ids"][0], skip_special_tokens=True
                )
            else:
                trunc_es[s_idx] = s
        return trunc_es
    ...
    def get_sorted_token_indices(self, inp, grad_pred_idx, pred_value):
        ...
        if is_multilabel and "signed" in self.masker.grad_type:
    ...
    def get_candidates(
        ...
        if self.predictor.model.config.problem_type == "multi_label_classification":
            pred_value = (
                1 if targ_pred_label in self.predictor_labels_to_ints.keys() else 0
            )
        ...
        if "t5" in self.tokenizer.name_or_path:
            edited_editable_segs = self._sample_edits(
                targ_pred_label,
                masked_inp,
                targ_pred_idx,
                num_spans=num_spans,
            ...
            edited_editable_segs = self._sample_edits_bert(
                targ_pred_label,
                masked_inp,
                targ_pred_idx,
                num_spans=num_spans,
            ...
            raise NotImplementedError(
                f"Model {self.editor_tok_wrapper.name_or_path} not implemented; \
    ...
    def _prepare_input_for_editor(
        self, inp, grad_pred_idx, sorted_token_indices=None, **kwargs
    ):
        ...
        if sorted_token_indices is not None:
            ...
            grouped_ind_to_mask, token_ind_to_mask, masked_inp, orig_spans = (
                self.masker.get_masked_string(
                    inp, editor_mask_indices=token_ind_to_mask, **kwargs
                )
            )
        else:
            grouped_ind_to_mask, token_ind_to_mask, masked_inp, orig_spans = (
    ...
    def _process_gen(self, masked_inp, gen, sentinel_toks):
        ...
        tokens = list(
            filter(None, re.split("<extra_id_.>|<extra_id_..>|<extra_id_...>", gen))
        )
        ...
        for idx, (token, sentinel_tok) in enumerate(tok_sentinel_iterator):
            sentinel_idx = (
                sentinel_tok[-2:-1] if len(sentinel_tok) == 12 else sentinel_tok[-3:-1]
            )
    ...
    def _get_pred_with_replacement(self, temp_gen, orig_spans):
        ...
        orig_tokens = list(
            filter(None, re.split("<extra_id_.>|<extra_id_..>", orig_spans))
        )
    ...
    def _sample_edits(
        ...
        while editable_segs != []:
            ...
            for inp_idx, (
                ...
                num_return_seqs = (
                    int(math.ceil(self.num_gens / num_inputs))
                    if num_sub_rounds != 0
                    else self.num_gens
                )
                ...
                masked_token_ids_tensor = (
                    torch.LongTensor(masked_token_ids + [eos_token_id])
                    .unsqueeze(0)
                    .to(self.device)
                )
                bad_tokens_ids = [[x] for x in range(sentinel_start, end_token_id)] + [
                    [eos_token_id]
                ]
                ...
                for batch_idx, batch in enumerate(batch_decoded):
                    sentinel_toks = [
                        f"<extra_id_{idx}>" for idx in range(0, span_end + 1)
                    ]
                    bad_gen, first_bad_tok, temp, stripped_batch = self._process_gen(
                        editable_seg, batch, sentinel_toks
                    )
                    ...
                    if bad_gen:
                        ...
                        sentinel_indices = np.nonzero(
                            ...
                            & (new_editable_token_ids <= sentinel_end)
                        )[0]
                        ...
                        new_input = self.get_editor_input(
                            targ_pred_label, new_editable_seg
                        )
                        ...
                        new_orig_token_idx = np.nonzero(
                            orig_token_ids == new_first_token
                        )
                        new_orig_token_ids = np.array(
                            orig_token_ids[new_orig_token_idx[0][0] :]
                        )
                        sentinel_indices = np.nonzero(
                            (new_orig_token_ids >= sentinel_start)
                            & (new_orig_token_ids <= sentinel_end)
                        )[0]
            ...
            for idx in unique_batch_indices:
                ...
                temp, edit_preds = self._get_pred_with_replacement(
                    new_editable_segs[idx], ot
                )
                if (
                    self.predictor.model.config.problem_type
                    == "multi_label_classification"
                ):
                    is_label = targ_pred_label in self.predictor_labels_to_ints.keys()
                    targ_pred_label = (
                        targ_pred_label if is_label else targ_pred_label[3:]
                    )
                ...
                targ_pred_idx = (
                    targ_pred_idx
                    if edit_labels[targ_pred_idx] == targ_pred_label
                    else edit_labels.index(targ_pred_label)
                )
                ...
                if predicted_label == contrast_label:
                    ...
                    self.predictor.model.config.problem_type
                    == "multi_label_classification"
                    and edit_probs[targ_pred_idx] < 0.5
                ):
                    edited_editable_segs.append(temp)

            highest_indices = np.argsort(targ_probs)[-k_intermediate:]
            filt_indices = [idx for idx in highest_indices if targ_probs[idx] != -1]
            ...
            masked_token_ids_lst = [
                new_masked_token_ids_lst[idx] for idx in filt_indices
            ]
        ...
        for idx, es in enumerate(edited_editable_segs):
            ...
            if len(matches) == 1:
                edited_editable_segs[idx] = re.sub(
                    r"<extra_id_([0-9]|[1-9][0-9])>", "", es
                )
            assert (
                len(matches) <= 1
    ...
    def _sample_edits_bert(
        ...
        while editable_segs != []:
            ...
            for inp_idx, (
                ...
                masked_token_ids_tensor = (
                    torch.LongTensor(masked_token_ids).unsqueeze(0).to(self.device)
                )
                ...
                batch_decoded = get_beam_prediction(
                    sent=editable_seg,
...
class RaceEditor(Editor):
    ...
    def _get_pred_with_replacement(self, temp_gen, orig_spans, inp):
        ...
        orig_tokens = list(
            filter(
                None, re.split("<extra_id_.>|<extra_id_..>|<extra_id_...>", orig_spans)
            )
        )
        ...
        temp_instance = self.tokenizer(
            inp["id"], temp_gen, inp["question"], inp["options"]
        )[0]
    ...
    def input_to_instance(self, inp, editable_seg=None, return_tuple=False):
        ...
        output = self.tokenizer(inp["id"], article, question, inp["options"])
        if return_tuple:
            return output
        return output[0]
    ...
    def _prepare_input_for_editor(
        self, inp, targ_pred_idx, grad_pred_idx, sorted_token_indices=None
    ):
        """Helper function that prepares masked input for Editor."""

        editable_seg = self.get_editable_seg_from_input(inp)

        tokens = [t.text for t in self.tokenizer_wrapper.tokenize(editable_seg)[:-1]]

        instance, length_lst, _ = self.input_to_instance(inp, return_tuple=True)
        editable_toks = get_predictor_tokenized(self.predictor, editable_seg)
        num_editab_toks = len(editable_toks)
        predic_tok_end_idx = len(editable_toks)
        predic_tok_end_idx = min(predic_tok_end_idx, length_lst[grad_pred_idx])
...
def get_beam_prediction(sent, tokenizer, model, k=2):
    ...
    for index, mask_index in tqdm(enumerate(masked_pos), total=len(masked_pos)):
        ...
        if len(curr_beam) == 0:
            curr_beam = list(
                zip(rankings, fill_mask_sentence(mask_index, idx, token_ids.squeeze()))
            )
        else:
            ...
            while len(topk_list) != 0:
                ...
                for index, cand in enumerate(curr_beam):
                    if cand == prev_beam[index] and cand[0] + value[0] > cand[0]:
                        curr_beam[index] = (
                            cand[0] + value[0],
                            fill_mask_sentence(mask_index, value[1], cand[1]),
                        )
                    elif (
                        cand != prev_beam[index]
                        and prev_beam[index][0] + value[0] > curr_beam[index][0]
                    ):
                        curr_beam[index] = (
                            prev_beam[index][0] + value[0],
                            fill_mask_sentence(
                                mask_index, value[1], prev_beam[index][1]
                            ),
                        )
...
def get_prediction_II(sent, tokenizer, model, device, k=2, fill_order="left"):
    ...
    token_ids = tokenizer.encode(sent, truncation=True, return_tensors="pt").to(device)
    masked_position = (token_ids.squeeze() == tokenizer.mask_token_id).nonzero()
    ...
    torch.cuda.empty_cache()
    return [
        tokenizer.decode(guess.squeeze()).replace("[CLS]", "").replace("[SEP]", "")
        for guess in list_of_ids
    ]
