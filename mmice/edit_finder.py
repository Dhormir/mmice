...
class EditEvaluator:
    def __init__(
        ...
        fluency_masker=RandomMasker(
            None,
            T5TokenizerFast.from_pretrained(
                "t5-small", model_max_length=700, legacy=False
            ),
            700,
        ),
    ):
        ...
        if "umt5" in fluency_model_name:
            self.fluency_model = UMT5ForConditionalGeneration.from_pretrained(
                fluency_model_name
            ).to(self.device)
        ...
        self.fluency_tokenizer = T5TokenizerFast.from_pretrained(
            fluency_model_name, model_max_length=700, legacy=False
        )
    ...
    def score_fluency(self, sent, batch_size=64):
        ...
        for batch in fluency_dataloader:
            input_ids = self.fluency_tokenizer(
                batch["text"],
                truncation=True,
                padding="max_length",
            ...
            labels.input_ids[
                labels.input_ids[:, :] == self.fluency_tokenizer.pad_token_id
            ] = -100
            labels = labels.to(self.device)
            outputs = self.fluency_model(
                input_ids=input_ids.input_ids, labels=labels.input_ids
            )
            loss = outputs[0]
    ...
    def score_minimality_embeddings(self, orig_sent, edited_sent, normalized=True):
        enc_orig_sent = self.fluency_tokenizer(orig_sent, return_tensors="pt").to(
            self.device
        )
        enc_edited_sent = self.fluency_tokenizer(edited_sent, return_tensors="pt").to(
            self.device
        )

        orig_sent_embeddings = self.fluency_model.encoder.embed_tokens(
            enc_orig_sent.input_ids
        )
        edited_sent_embeddings = self.fluency_model.encoder.embed_tokens(
            enc_edited_sent.input_ids
        )
        cos = torch.nn.CosineSimilarity(dim=1)
        # We use the centroid of each sentence and detemine how similar they are
        output = cos(
            orig_sent_embeddings.sum(dim=1) / orig_sent_embeddings.size()[1],
    ...
    def score_minimality_mauve(self, orig_sent: str, edited_sent: str, normalized=True):
        ...
        similarity = compute_mauve(
            ...
            max_text_length=512,
            device_id=1,
            featurize_model_name="FacebookAI/xlm-roberta-base",
            batch_size=12,
            verbose=False,
        )
...
def get_scores(predictor, instance_candidates, contrast_pred_idx, k=None):
    ...
    if k:
        ...
        cpu_contrast_probs = (
            torch.index_select(contrast_probs, 0, highest_indices).cpu().numpy()
        )
...
class EditFinder:
    ...
    def score_minimality(
        self, edit_evaluator: EditEvaluator, orig_sent: str, edited_sent: str
    ):
    ...
    def run_edit_round(
        ...
        logger.info(
            wrap_text(
                f"Running candidate generation for mask frac: \
        ...
        instance_cands = [
            es if es else inp for inp, es in zip(input_cands, editable_seg_cands)
        ]
        # logger.info(f'instance_cands length:\n{len(instance_cands)}')
        instance_cands = self.predictor.tokenizer(
            instance_cands, padding=True, truncation=True, return_tensors="pt"
        )
        ...
        for sort_idx, (prob, pred_idx, input_cand, editable_seg_cand) in iterator:
            if self.verbose and sort_idx == 0:
                logger.info(
                    wrap_text(
                        f"Post edit round, contr label: {edit_list.contrast_label}"
                    )
                )
            ...
            if is_multilabel and not is_label:
                label = "NO " + label
            if (pred_idx == contrast_pred_idx and not is_multilabel) or (
                pred_idx == contrast_pred_idx and is_multilabel and prob > 0.5
            ):
                found_cand = True
    ...
    def binary_search_edit(
        ...
        if num_levels == max_levels:
            ...
            error_msg = (
                "Reached > max binary search levels." + f"({num_levels} > {max_levels})"
            )
    ...
    def linear_search_edit(
        ...
        mask_frac_iterator = np.arange(
            min_mask_frac + mask_frac_step,
    ...
    def minimally_edit(
        ...
        if self.predictor.model.config.problem_type == "multi_label_classification":
            orig_pred_label = (
                orig_preds[0]["label"]
                if orig_pred_prob >= 0.5
                else "NO " + orig_preds[0]["label"]
            )
            contrast_label = (
                "NO " + orig_preds[0]["label"]
                if orig_pred_prob >= 0.5
                else orig_preds[0]["label"]
            )
            orig_contrast_prob = (
                1 - orig_pred_prob if orig_pred_prob >= 0.5 else orig_pred_prob
            )
        ...
        while new_pred_label != contrast_label:
            ...
            for beam_elem_idx, (score, _, input_cand) in iterator:
                ...
                sorted_token_indices = self.editor.get_sorted_token_indices(
                    input_cand, pred_idx, orig_pred_prob
                )
                ...
                if self.search_method == "binary":
                    ...
                    self.linear_search_edit(
                        edit_list,
                        input_cand,
                        self.labels_to_ints[contrast_label],
                        num_rounds,
                        max_mask_frac=self.max_mask_frac,
                        sorted_token_indices=sorted_token_indices,
                        edit_evaluator=edit_evaluator,
