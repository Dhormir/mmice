...
class StageOneDataset(Dataset):
    ...
    def create_inputs(
        ...
        for i, (orig_inp, orig_label) in tqdm(
            ...
            if predictor.model.config.problem_type == "multi_label_classification":
                ...
                label_idx = (
                    self.multilabels_to_array(label_to_use, labels_to_ints)
                    if not are_disjoint
                    else label_to_use
                )
                label_to_use = self.array_to_multilabels(
                    label_idx, predictor.model.config.id2label
                )
            else:
                # If its not in mapping we assume it's because it is already encoded and therefore we do nothing
                label_idx = (
                    labels_to_ints[label_to_use]
            ...
            try:
                ...
                mask_string = lambda pred: masker.get_masked_string(
                    orig_inp,
                    pred_idx=pred[0],
                    pred_value=pred[1],
                ...
                map_format_input = map(
                    format_input,
                    zip(
                        [masked_output[0] for masked_output in masked_outputs],
                        label_to_use,
                    ),
                )
                ...
                if verbose:
                    ...
                    logger.info(
                        wrap_text(
                            f"Problem type: {predictor.model.config.problem_type}"
                        )
                    )
                    logger.info(wrap_text(f"labels_to_ints: {labels_to_ints}"))
                    logger.info(wrap_text(f"label_idx: {label_idx}"))
                    logger.info(wrap_text(f"label to use: {label_to_use}"))
                    logger.info(wrap_text(f"Original input ({i}): " + orig_inp))
                    logger.info(wrap_text(f"Mask frac: {rounded_mask_frac}"))
                    logger.info(wrap_text(f"Editor input: {masked_strings_[0]}"))
                    logger.info(wrap_text(f"Editor target: {targets_[0]}"))
...
class RaceStageOneDataset(StageOneDataset):
    ...
    def create_inputs(
        ...
        if editable_key not in editable_keys:
            raise ValueError(
                f"Editable key must be in {editable_keys} \
        ...
        for i, (orig_inp, gold_label) in tqdm(iterator, total=len(orig_inputs)):
            ...
            instance, length_lst, _ = dataset_reader.text_to_instance(
                orig_inp["id"],
                orig_inp["article"],
                orig_inp["question"],
                orig_inp["options"],
            ...
            try:
                ...
                if editable_key == "article":
                    article_tok = get_predictor_tokenized(
                        predictor, orig_inp["article"]
                    )
                    predictor_tok_end_idx = min(len(article_tok), length_lst[label_idx])
                    _, _, masked_article, target = masker.get_masked_string(
                        orig_inp["article"],
                        label_idx,
                ...
                # TODO: Does this work? Have only tested article
                    ...
                    question_tok = get_predictor_tokenized(
                        predictor, orig_inp["question"]
                    )
                    predictor_tok_end_idx = length_lst[label_idx] + len(question_tok)
                    _, _, masked_question, target = masker.get_masked_string(
                        orig_inp["question"],
                        label_idx,
                        labeled_instance=instance,
