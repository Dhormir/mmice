...
def train_epoch(epoch, editor_tokenizer, editor_model, train_data_loader, optimizer):
    ...
    logger.info(f"Epoch: {epoch}," + f" Avg Batch Loss:  {avg_loss}")
    return avg_loss
...
def validate_epoch(epoch, editor_tokenizer, editor_model, val_data_loader):
    ...
    logger.info(f"Epoch: {epoch}," + f" Avg Validation Batch Loss:  {avg_loss}")
    return avg_loss
...
def get_datasets(
    ...
    # Else, create data by calling create_inputs() function in dataset.py
        ...
        if args.meta.task == "race":
            ...
            val_dataset = RaceStageOneDataset(
                editor_tokenizer,
                max_length=args.model.model_max_length,
                lang=args.meta.lang,
            )
...
def run_train_editor(predictor, dataset_reader, args):
    ...
    optimizer = ACCELERATOR.prepare(
        torch.optim.Adam(params=editor_model.parameters(), lr=args.train.lr)
    )
    ...
    progress_bar = tqdm(
        range(args.train.num_epochs), disable=not ACCELERATOR.is_local_main_process
    )
    ...
    for epoch in progress_bar:
        ...
        if os.path.exists(path):
            ...
            avg_train_loss = train_epoch(
                epoch, editor_tokenizer, editor_model, train_data_loader, optimizer
            )
            logger.info(
                f"Epoch {epoch} Avg Train Batch Loss: {avg_train_loss:.4f}. Saving weights to: {path}"
            )
            logger.info("Saving Editor checkpoint to: " + path)

            ACCELERATOR.save_state(path, safe_serialization=False)
            val_loss = validate_epoch(
                epoch, editor_tokenizer, editor_model, val_data_loader
            )
            ...
            progress_bar.set_postfix_str(
                f"Avg Train Loss: {avg_train_loss:.4f}, Best Validation Loss: {best_val_loss:.4f}"
            )
    progress_bar.close()
